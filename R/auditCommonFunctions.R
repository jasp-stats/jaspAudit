...
.jfaAddAuditRiskModel <- function(options, jaspResults, position) {
  ...
  if (!(options[["ir"]] == "custom" && options[["cr"]] == "custom" && options[["car"]] == "custom")) {
    ...
    tb$addColumnInfo(name = "ir", title = gettext("Inherent risk"), type = "number", format = "pc;dp:0")
    tb$addColumnInfo(name = "cr", title = gettext("Control risk"), type = "number", format = "pc;dp:0")
    tb$addColumnInfo(name = "car", title = gettext("Analytical risk"), type = "number", format = "pc;dp:0")
    tb$position <- 4
