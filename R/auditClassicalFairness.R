...
.jfaGetFairnessMetricInfo <- function(options, jaspResults) {
  out <- list()
  if (options[["workflow"]]) {
    out[["metric"]] <- jaspResults[["selectionState"]]$object[["measure"]]
  } else {
...
.jfaFairnessAddProcedure <- function(options, jaspResults, position) {
  if (options[["explanatoryText"]] && is.null(jaspResults[["procedureContainer"]])) {
    ...
    metric <- .jfaGetFairnessMetricInfo(options, jaspResults)
    if (metric[["metric"]] == "eo") {
  ...
  if (!is.null(jaspResults[["state"]])) {
    ...
    tryResult <- try({
      if (metric[["metric"]] != "eo") {
        ...
        #  metric = .jfaGetFairnessMetricInfo(options, jaspResults)[["metric"]], alternative = options[["alternative"]],
...
.jfaFairnessComparisonsTable <- function(dataset, options, fairnessContainer, jaspResults, ready, positionInContainer) {
  ...
  metric <- .jfaGetFairnessMetricInfo(options, jaspResults)
  pvalMessage <- gettext("<i>p</i>-values are uncorrected for multiple comparisons and are based on Fisher's exact test.")
  ...
  if (metric[["metric"]] != "eo") {
    ...
    if (is.null(fairnessContainer[["comparisonsTable"]])) {
      ...
      # )
      # tb[["bf"]] <- c(NA, bfs)
      tb$addFootnote(pvalMessage, colName = "p")
    ...
    if (is.null(container[["comparisonsTableTpr"]])) {
      ...
      message <- switch(options[["alternative"]],
        "two.sided" = gettext("The null hypothesis specifies that the true positive rate of an unprivileged group is equal to that of the privileged (P) group."),
        "less" = gettext("The null hypothesis specifies that the true positive rate of an unprivileged group is greater than, or equal to, that of the privileged (P) group."),
        "greater" = gettext("The null hypothesis specifies that the true positive rate of an unprivileged group is lower than, or equal to, that of the privileged (P) group.")
      )
      ...
      message <- switch(options[["alternative"]],
        "two.sided" = gettext("The null hypothesis specifies that the false positive rate of an unprivileged group is equal to that of the privileged (P) group."),
        "less" = gettext("The null hypothesis specifies that the false positive rate of an unprivileged group is greater than, or equal to, that of the privileged (P) group."),
        "greater" = gettext("The null hypothesis specifies that the false positive rate of an unprivileged group is lower than, or equal to, that of the privileged (P) group.")
      )
      ...
      tb1$addFootnote(pvalMessage, colName = "p")
      ...
      tb2$addFootnote(pvalMessage, colName = "p")
    }
  }
}
...
.jfaFairnessConfusionTable <- function(dataset, options, fairnessContainer, jaspResults, ready, positionInContainer) {
  ...
  if (is.null(fairnessContainer[["confusionTable"]])) {
    ...
    if (ready) {
      ...
      if (metric[["metric"]] == "eo") {
        tb$addColumnInfo(name = "fmetric", title = .jfaGetFairnessMetricInfo(options, jaspResults)[["title"]][[1]], type = if (.jfaGetFairnessMetricInfo(options, jaspResults)[["metric"]] == "pp") "integer" else "number")
        tb$addColumnInfo(name = "smetric", title = .jfaGetFairnessMetricInfo(options, jaspResults)[["title"]][[2]], type = if (.jfaGetFairnessMetricInfo(options, jaspResults)[["metric"]] == "pp") "integer" else "number")
        result <- .jfaFairnessState(dataset, options, jaspResults)[["frequentist"]][["tp"]]
        ...
        tb$addFootnote(gettextf("%1$s = %2$s", .jfaGetFairnessMetricInfo(options, jaspResults)[["title"]][[1]], .jfaGetFairnessMetricInfo(options, jaspResults)[["formula"]][[1]]), colName = "fmetric")
        tb$addFootnote(gettextf("%1$s = %2$s", .jfaGetFairnessMetricInfo(options, jaspResults)[["title"]][[2]], .jfaGetFairnessMetricInfo(options, jaspResults)[["formula"]][[2]]), colName = "smetric")
      } else {
        tb$addColumnInfo(name = "metric", title = .jfaGetFairnessMetricInfo(options, jaspResults)[["title"]], type = "number")
        est <- result[["metric"]][["all"]][["estimate"]]
        after <- 1
        for (i in seq_along(groupLevels)) {
          est <- append(est, rep(NA, length(factorLevels) - 1), after = after)
        ...
        tb[["metric"]] <- est
        tb$addFootnote(gettextf("True Positives (TP) are cells in which the positive class (<i>%1$s</i>) is correctly predicted, while True Negatives (TN) are cells where the negative class is correctly predicted. False Negatives (FN) are cells in which the positive class is incorrectly predicted, and False Positives (FP) are cells in which the negative class is incorrectly predicted.", options[["positive"]]))
        tb$addFootnote(gettextf("%1$s = %2$s", .jfaGetFairnessMetricInfo(options, jaspResults)[["title"]], .jfaGetFairnessMetricInfo(options, jaspResults)[["formula"]]), colName = "metric")
      }
    } else {
      if (!options[["confusionTableTransposed"]]) {
...
.jfaFairnessParityPlot <- function(dataset, options, fairnessContainer, jaspResults, ready, positionInContainer) {
  ...
  if (metric[["metric"]] != "eo") {
    ...
    if (options[["explanatoryText"]]) {
      caption <- createJaspHtml(gettextf("<b>Figure %1$i.</b> Estimated parity statistics (i.e., the ratio of %2$s from each unprivileged group to the privileged %3$s group) for all groups in the data. The closer the parity lies to one, the less discrimination occurs in the algorithm.", jaspResults[["figNumber"]]$object, tolower(.jfaGetFairnessMetricInfo(options, jaspResults)[["title"]]), options[["privileged"]]), "p")
...
.jfaFairnessAddConclusion <- function(options, fairnessContainer, jaspResults, ready, position) {
  if (!is.null(jaspResults[["conclusionContainer"]]) || !ready || !options[["explanatoryText"]]) {
    ...
    rejectedText <- gettext("is rejected")
    notRejectedText <- gettext("is not rejected")
    conclusionRejectedText <- gettext("As the measures are significantly different from each other, we can conclude that this algorithm is unfair against the unprivileged group(s).")
    conclusionNotRejectedText <- gettext("As the measures are not significantly different from each other, we cannot conclude that this algorithm is unfair against the unprivileged group(s).")
    container$position <- position
    ...
    if (metric[["metric"]] == "eo") {
      ...
      pval <- if (rejectnull) paste0(pval, " < \u03B1") else paste0(pval, " >= \u03B1")
      caption <- gettextf("The <i>p</i>-value is %1$s and the null hypothesis of equal %2$s across protected groups %3$s. %4$s", pval, tolower(metric[["title"]]), conclusion_part1, conclusion_part2)
