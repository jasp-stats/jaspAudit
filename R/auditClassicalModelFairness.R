  ...
  if (is.null(fairnessContainer[["confusionTable"]])) {
    ...
    if (ready) {
      ...
      for (i in seq_along(colnames(privConfTable))) {
        ...
        tb$addColumnInfo(name = "group", title = "", type = "string")
        tb$addColumnInfo(name = "varname_pred", title = gettext("Predicted Values"), type = "string")
        ...
        for (i in seq_along(rownames(privConfTable))) {
           ...
           tb$addColumnInfo(name = name, title = rownames(privConfTable)[i], type = colType, overtitle = gettext("Observed Values"))
           col <- numeric()
           for (j in seq_along(groupLevels)) {
            values <- result[["confusion.matrix"]][[j]]$matrix[i,]
            if (options[["confusionTableProportions"]]) {
...
if (is.null(fairnessContainer[["parityPlot"]])) {
  ...
  if (options[["explanatoryText"]]) {
    caption1 <- createJaspHtml(gettextf("<b>Figure 1.</b> Estimated parity statistics (i.e., the ratio of true positive rate from each unprivileged group to the privileged %3$s group) for all groups in the data. The closer the parity lies to one, the less discrimination occurs in the algorithm.", jaspResults[["figNumber"]]$object, tolower(.jfaFairnessGetMetricFromQuestion(options)[["title"]]), options[["privileged"]]), "p")
  ...
  container[["parityPlot2"]] <- plot2
   ...
   if (options[["explanatoryText"]]) {
    caption2 <- createJaspHtml(gettextf("<b>Figure 2.</b> Estimated parity statistics (i.e., the ratio of false positive rate from each unprivileged group to the privileged %3$s group) for all groups in the data. The closer the parity lies to one, the less discrimination occurs in the algorithm.", jaspResults[["figNumber"]]$object, tolower(.jfaFairnessGetMetricFromQuestion(options)[["title"]]), options[["privileged"]]), "p")
...
.jfaFairnessAddConclusion <- function(options, fairnessContainer, jaspResults, 
  ...
  rejectnull <- state[["frequentist"]][["parity"]][["all"]][2, "estimate"] < 0.80
  conclusion <- if (rejectnull) gettext("lower") else gettext("higher")
  conclusion2 <- if (rejectnull) gettext(" We can conclude that using this specific AI leads to discriminatory acts against the unprivileged group and is, therefore, not fair.")  else gettext("We cannot conclude that using this specific AI leads to discriminatory acts against the unprivileged group. ")
  
  caption <- gettextf("The use of Disparate Impact as a fairness measure required a deterministic comparison approach, adhering to the so-called 80-percent rule, to draw a conclusion regarding the algorithm's fairness. According to this rule, the Protected-to-Privileged Positive Rate Ratio (Table 1) is compared against a decision threshold of 0.80. The Protected-to-Privileged Positive Rate Ratio is %1$s than 0.80. %2$s",conclusion, conclusion2)
  ...
  conclusion2 <- if (rejectnulla || rejectnullb) gettext(" As the two values are significantly different from each other, we can conclude that using this specific AI leads to discriminatory acts against the unprivileged group and is, therefore, not fair.")  else gettext("As the two values are not significantly different from each other, we cannot conclude that using this specific AI leads to discriminatory acts against the unprivileged group. ")
  ...
  caption <- gettextf("The <i>p</i>-value are %1$s and %2$s the null hypothesis of equal %3$s and %4$s across protected groups %5$s. %6$s ", pvalue1, pvalue2, tolower(.jfaFairnessGetMetricFromQuestion(options)[["title"]][[1]]) , tolower(.jfaFairnessGetMetricFromQuestion(options)[["title"]][[2]]), conclusion, conclusion2)
  }else {
  state <- .jfaFairnessState(dataset = NULL, options, jaspResults)
  ...
  conclusion2 <- if (rejectnull) gettext(" As the two values are significantly different from each other, we can conclude that using this specific AI leads to discriminatory acts against the unprivileged group and is, therefore, not fair.")  else gettext("As the two values are not significantly different from each other, we cannot conclude that using this specific AI leads to discriminatory acts against the unprivileged group. ")
