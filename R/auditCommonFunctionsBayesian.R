...
.jfaPlotPriorAndPosterior <- function(options, parentOptions, parentState, parentContainer, jaspResults, 
  ...
  if (is.null(parentContainer[["plotPriorAndPosterior"]])) {
    ...
    if (info && stage == "evaluation") {
      ...
      if (options[["materiality_test"]]) {
        ...
        plot_middle <- text_left <- ggplot2::ggplot() + jaspGraphs::getEmptyTheme()
      }
  ...
  if (options[["explanatoryText"]]) {
    method <- if (stage == "planning") options[["likelihood"]] else options[["method"]]
    distribution <- switch(method, "poisson" = gettext("gamma"), "binomial" = gettext("beta"), "hypergeometric" = gettext("beta-binomial"))
    if (stage == "planning") {
      ...
      figureCaption <- createJaspHtml(gettextf("<b>Figure %1$i.</b> The prior and expected posterior distribution (%2$s) on the population misstatement \u03B8. The prior parameters <i>%3$s = %4$s, %5$s = %6$s</i> are derived from the prior information. %7$s",  
                                               ...
                                               if (options[["materiality_test"]]) additionalText1 else ""), "p")
    } else {
      additionalText <- gettext("The gray dots represent the performance materiality.")
      figureCaption <- createJaspHtml(gettextf("<b>Figure %1$i.</b> The prior and posterior distribution (%2$s) on the misstatement in the population. %3$s",
                                               jaspResults[["figNumber"]]$object,
                                               distribution,
                                               if (options[["plotPosteriorInfo"]] && options[["materiality_test"]]) additionalText else ""), "p")
    }
...
.jfaPlotPredictive <- function(options, parentOptions, parentState, parentContainer, jaspResults, 
  ...
  if (options[["explanatoryText"]]) {
    object <- if (stage == "planning") parentState[["prior"]] else parentState[["posterior"]]
	size <- if (stage == "planning") parentState[["n"]] else parentState[["N.units"]] - parentState[["n"]]
    figureCaption <- createJaspHtml(gettextf("<b>Figure %1$i.</b> The %2$s predictive distribution is %3$s and displays the predictions of the %2$s distribution for %4$s <i>n</i> = %5$s.",
                                             jaspResults[["figNumber"]]$object,
...
.jfaTablePriorPosterior <- function(options, parentOptions, parentState, parentContainer, jaspResults, 
  ...
  if (is.null(parentContainer[["tablePriorPosterior"]])) {
    ...
    if (options[["materiality_test"]]) {
      rows <- cbind(rows, hMin = c(prior[["hypotheses"]][["p.h1"]], posterior[["hypotheses"]][["p.h1"]], posterior[["hypotheses"]][["p.h1"]] / prior[["hypotheses"]][["p.h1"]]),
